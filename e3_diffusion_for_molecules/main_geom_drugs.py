...
## MAY NEED TO EDIT SCRIPT FOR MULTI GPU TRAINGING
...
if (not args.use_canon):
    optim = get_optim(args, model)
else:
    optim = get_optim(args, model, canonicalization_network)# print(model)
# print(canonicalization_network)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        train_test.train_epoch(args, dataloaders['train'], epoch, model, model_dp, model_ema, ema, device, dtype,
                               ...
                               prop_dist, canonicalizer = canonicalizer)
        ...
        if epoch % args.test_epochs == 0:
            ...
            nll_val = train_test.test(args, dataloaders['val'], epoch, model_ema_dp, device, dtype,
                                      property_norms, nodes_dist, partition='Val', canonicalizer=canonicalizer)
            nll_test = train_test.test(args, dataloaders['test'], epoch, model_ema_dp, device, dtype,
                                       property_norms, nodes_dist, partition='Test', canonicalizer=canonicalizer)
            ...
            if args.save_model:
                ...
                if canonicalizer != None:
                    utils.save_model(canonicalization_network, 'outputs/%s/canonicalization_network_%d.npy' % (args.exp_name, epoch))
