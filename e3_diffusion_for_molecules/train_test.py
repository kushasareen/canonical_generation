...
import time
import torch
import channels_egnn.qm9.utils as channels_egnn_utils
from canonicalization.utils import run_canonicalization
from qm9.visualizer import plot_data3d

# need to modify to use position coords and the canonicalizer network
def train_epoch(args, loader, epoch, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
                nodes_dist, gradnorm_queue, dataset_info, prop_dist, canonicalizer = None, break_sym = False):
    model_dp.train()
    model.train()
    nll_epoch = []
    ...
    for i, data in enumerate(loader):
        ...
        if args.clip_grad:
            ...
            if canonicalizer != None:
                utils.gradient_clipping(canonicalizer.canonicalization_network, gradnorm_queue)
        else:
            grad_norm = 0.
...
def test(args, loader, epoch, eval_model, device, dtype, property_norms, nodes_dist, partition='Test', canonicalizer = None, break_sym = None):
    eval_model.eval()
    with torch.no_grad():
        nll_epoch = 0
        n_samples = 0
